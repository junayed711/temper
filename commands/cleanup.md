...
temper item whose branch name, or path inside the repo, has a character outside
`A-Za-z0-9._/-`; say it's for the user to remove by hand.
...
- **PR**: `gh pr list --head <branch> --state all --json number,state` gives open,
  merged, closed, or none. Without `gh`, "PR unknown".
...
- **worktree**: none; "folder missing" if the porcelain record says `prunable`;
  ...
  gives "no changes" or "uncommitted changes".
...
changes, a locked worktree, or anything that couldn't be checked.
...
Delete nothing they didn't name.

Done when the user has answered.

## 4. Delete

For each number named, in order:
